...
inline float anchor_frequency_for_band(float band) {
    if (band <= kBandAnchors.front().band) return kBandAnchors.front().hz;
    if (band >= kBandAnchors.back().band) return kBandAnchors.back().hz;
...
inline BandLayout make_we_layout(float sample_rate) {
    ...
    for (std::size_t k = 0; k < kNumBins; ++k) {
        ...
        if (k > 0 && next <= layout.edges[k - 1]) next = layout.edges[k - 1] + 1;
...
inline float shape_response(float unit) {
    ...
    if (x <= 0.5f) return 0.5f * std::pow(x * 2.0f, kResponseContrast);
