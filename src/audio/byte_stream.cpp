...
auto PosixFile::open(const std::string& path) -> rstd::io::Result<std::unique_ptr<PosixFile>> {
...
auto PosixFile::read(rstd::u8* buf, rstd::usize len) -> rstd::io::Result<rstd::usize> {
    while (true) {
...
auto PosixFile::seek(SeekFrom pos) -> rstd::io::Result<rstd::u64> {
    int whence;
    switch (pos.which) {
