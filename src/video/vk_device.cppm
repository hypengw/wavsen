...
public:
    ...
    const std::vector<const char*>& enabled_instance_extensions() const {
        return enabled_inst_exts_;
    }
    ...
    auto upload_into(VkImage target, std::uint32_t target_width, std::uint32_t target_height,
                     const std::uint8_t* data, std::size_t size) -> rstd::Result<int, Error>;
...
private:
    ...
    static std::unique_ptr<Producer> build_(std::uint32_t width, std::uint32_t height,
