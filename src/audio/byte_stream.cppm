...
public:
    ...
    virtual auto read(rstd::u8* buf, rstd::usize len) -> rstd::io::Result<rstd::usize> = 0;

    // Seek to a position. Returns the resulting absolute byte offset.
    virtual auto seek(rstd::io::SeekFrom pos) -> rstd::io::Result<rstd::u64> = 0;
};
...
public:
    static auto open(const std::string& path) -> rstd::io::Result<std::unique_ptr<PosixFile>>;

    ~PosixFile() override;
    PosixFile(const PosixFile&)            = delete;
    PosixFile& operator=(const PosixFile&) = delete;

    auto read(rstd::u8* buf, rstd::usize len) -> rstd::io::Result<rstd::usize> override;
